...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QCheckBox, QSpinBox, QDoubleSpinBox, QLabel, QPushButton, QComboBox, QFormLayout, QDialog, QScrollArea
...
class DeltaPDFOptionsWidget(QDialog):
    ...
    def __init__(self, nxdata:NXdata, parent=None):
        ...
        self.nxdata = nxdata
        self.dpdf = DeltaPDF()
        self.dpdf.set_data(nxdata)
        
        self.originalDpdf = DeltaPDF()
        self.originalDpdf.set_data(nxdata)
    ...
    def getDeltaPDF(self):
        self.originalDpdf.set_lattice_params((self.aSpinBox.value(), self.bSpinBox.value(), self.cSpinBox.value(), self.alphaSpinBox.value(), self.betaSpinBox.value(), self.gammaSpinBox.value()))
        
        mask = self.generateMask(self.originalDpdf)
        if mask is not None:
            self.originalDpdf.add_mask(mask)
            self.originalDpdf.punch()
            
        if self.kernalComboBox.currentText() != "Select Kernal":
            if self.kernalComboBox.currentText() == "Gaussian":
                ...
                self.originalDpdf.set_kernel(Gaussian3DKernel(stddev=self.gaussianStdev.value(), size=size))      
                self.originalDpdf.interpolate()  
                
        txt = self.taperComboBox.currentText()
        if txt == "Select Window":
            pass
        elif txt == "Tukey":
            alpha = (self.tukeyAlphaH.value(), self.tukeyAlphaK.value(), self.tukeyAlphaL.value())
            try:
                self.originalDpdf.set_tukey_window(turkey_alphas=alpha)
            except TypeError:
                self.originalDpdf.set_tukey_window()
            ...
            try:
                ...
                self.originalDpdf.set_ellipsoidal_tukey_window(turkey_alphas=alpha)
            except TypeError:
                # fallback to older 3-arg signature (H,K,L)
                try:
                    self.originalDpdf.set_ellipsoidal_tukey_window(turkey_alphas=(alpha[0], alpha[2], alpha[4]))
                except Exception:
                    pass
        elif txt == "Hexagonal":
            try:
                self.originalDpdf.set_hexagonal_tukey_window(turkey_alphas=(self.hexAlphaH.value(), self.hexAlphaK.value(), self.hexAlphaHK.value(), self.hexAlphaL.value()))
            except TypeError:
                self.originalDpdf.set_hexagonal_tukey_window(turkey_alphas=self.hexAlphaH.value()) 
        ...
        if padding != (0,0,0):
            ...
            self.originalDpdf.pad(padding=padding)                      
            
        self.originalDpdf.perform_fft()
        return self.originalDpdf
    ...
    def generate_mask_preview(self):
        ...
        mask = self.generateMask(self.dpdf)
        if mask is not None:
            self.maskPlotPreview.updatePColorMeshPlot(mask[:,:,mask.shape[2]//2].transpose())
            self.maskPlotPreview.ax_main.set_aspect(self.dpdf.lattice_params[1]/self.dpdf.lattice_params[0])
        else:
    ...
    def generate_kernal_preview(self):
        ...
        if self.kernalComboBox.currentText() == "Gaussian":
            ...
            size = (int(self.gaussianSizeX.value()), int(self.gaussianSizeY.value()), int(self.gaussianSizeZ.value()))
            self.dpdf.set_kernel(Gaussian3DKernel(stddev=self.gaussianStdev.value(), size=size))
            self.dpdf.interpolate()
            try:
                quadmesh = plot_slice(self.dpdf.interpolated[:,:,0.0])
    ...
    def generate_taper_preview(self):
        ...
        if txt == "Tukey":
            alpha = (self.tukeyAlphaH.value(), self.tukeyAlphaK.value(), self.tukeyAlphaL.value())
            try:
                self.dpdf.set_tukey_window(turkey_alphas=alpha)
            except TypeError:
                self.dpdf.set_tukey_window()
        elif txt == "Ellipsoidal":
            alpha = (
                self.ellAlphaH.value(),
                self.ellAlphaHK.value(),
            ...
            try:
                # prefer a single alpha tuple if supported
                self.dpdf.set_ellipsoidal_tukey_window(turkey_alphas=alpha)
            except TypeError:
                # fallback to older 3-arg signature (H,K,L)
                try:
                    self.dpdf.set_ellipsoidal_tukey_window(turkey_alphas=(alpha[0], alpha[2], alpha[4]))
                except Exception:
                    pass
        elif txt == "Hexagonal":
            try:
                self.dpdf.set_hexagonal_tukey_window(turkey_alphas=(self.hexAlphaH.value(), self.hexAlphaK.value(), self.hexAlphaHK.value(), self.hexAlphaL.value()))
            except TypeError:
                self.dpdf.set_hexagonal_tukey_window(turkey_alphas=self.hexAlphaH.value())
        self.taperPlotPreview.setVisible(True)
        if getattr(self.dpdf, 'window', None) is not None:
            try:
                window = self.dpdf.window
    ...
    def generate_padding_preview(self):
        ...
        if padding != (0, 0, 0):
            self.paddingTriple = padding
            self.dpdf.pad(padding=padding)
            self.paddingPlotPreview.setVisible(True)
            if getattr(self.dpdf, 'padded', None) is not None:
                try:
                    padded = self.dpdf.padded
